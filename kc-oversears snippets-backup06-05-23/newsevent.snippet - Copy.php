...
<div class="news_event_section_filter card_section_filter">
	<div class="news_filter_left"> 
        ...
        <?php foreach($nwsTerms as $nwsTerm ){ ?>           
			<li onclick="getMyNews(this)" id="<?php echo $nwsTerm->term_id; ?>" class="<?php echo ( $nwCrt == 0 )? "myactivenews" : "" ?>"><span><?php echo $nwsTerm->name; ?></span></li>
        <?php $nwCrt++; } ?>			
		</ul>
	</div>
	<div class="news_filter_right"> 		
        <input type="text" name="searchp" id="searchp" placeholder="Search News" onkeydown="newEventFilter()" />
...
function getMyNews(newsid){    
    jQuery('.filter_tabs li').removeClass('myactivenews');
    jQuery(newsid).addClass('myactivenews');
...
function newEventFilter(newsid,pgum){
    ...
    var newsval = jQuery ('.myactivenews').attr('id');       
            ...
            if (resp.status == 'success') {
                ...
                if (datas >= 1) {
                    ...
                    for (i = 0; i < datas; i++) {                        
                        ...
                        <a href="${resp.data[i].permalink}">Read More</a>
                        </div>
                        </div>
                        </div>
                        </div>  `;                       
